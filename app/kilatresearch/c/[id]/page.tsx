'use client';
/**
 * KilatResearch - Deep Research Hub (Perplexity-style)
...
export default function KilatResearchPage({ params }: PageProps) {
    ...
    const [projectName, setProjectName] = useState('research');
    ...
    useEffect(() => {
        if (!isProcessing && messages.length > 0 && messages[messages.length - 1]?.status === 'complete') {
            const timer = setTimeout(() => setSuggestions(getPostTaskSuggestions('research')), 500);
    ...
    const handleSendMessage = useCallback(async (content: string, attachments?: { type: 'image' | 'document'; name: string; base64: string }[]) => {
        ...
        const assistantMsg: Message = { id: `msg_${Date.now()}_assistant`, role: 'assistant', content: '🔬 Researching...', timestamp: Date.now(), agent: 'KilatResearch', status: 'streaming' };
        setMessages(prev => [...prev, assistantMsg]);

        try {
            const { jobId } = await (await fetch('/api/kilat/async', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: content, userId: user!.id, sessionId: projectId, mode: 'planning', model: selectedModel, agentType: 'research' }) })).json();
            ...
            while (!completed && pollCount < 300) {
                ...
                if (statusData.job?.status === 'completed') {
                    completed = true;
                    setMessages(prev => prev.map(m => m.id === assistantMsg.id ? { ...m, content: statusData.job.result?.content || 'Research complete!', status: 'complete' as const } : m));
                    if (statusData.job.result?.citations) setCitations(statusData.job.result.citations);
    ...
    const handleNewProject = useCallback(() => router.push(`/kilatresearch/c/${crypto.randomUUID()}`), [router]);
        ...
        <div className="flex flex-col h-screen overflow-hidden bg-background-dark font-sans text-slate-200">
            ...
            <main className="flex flex-1 overflow-hidden relative">
                ...
                <ChatPanel sessionId={projectId} messages={messages} isProcessing={isProcessing} chatMode="planning" selectedModel={selectedModel}
                    ...
                    availableModels={availableModels} onSessionSelect={(id) => router.push(`/kilatresearch/c/${id}`)} onNewChat={handleNewProject}
                    isCollapsed={isChatCollapsed} onToggleCollapse={() => setIsChatCollapsed(p => !p)} agentType="research" />
                ...
                <div className="w-80 border-l border-white/10 p-4 overflow-y-auto">
                    <h3 className="text-xs font-bold uppercase text-slate-500 mb-4">Sources ({citations.length})</h3>
                    {citations.length === 0 ? <p className="text-sm text-slate-500">Ask a question to see sources</p> :
                        citations.map((c, i) => (
                            <a key={i} href={c.url} target="_blank" rel="noopener noreferrer" className="block p-3 rounded-lg bg-white/5 hover:bg-white/10 mb-2">
                                <p className="text-sm font-medium text-primary truncate">{c.title}</p>
                                <p className="text-xs text-slate-400 mt-1 line-clamp-2">{c.snippet}</p>
                            </a>
                        ))
                    }
                </div>
